...
void main(){
    ...
    vec2 tex_co_modified_r = tex_coord;
    tex_co_modified_r.x = tex_co_modified_r.x - STRENGTH;
    vec2 tex_co_modified_g = tex_coord;
    tex_co_modified_g.x = tex_co_modified_g.x + STRENGTH;
    vec4 rValue = texture2D(tex, tex_co_modified_r);  
    vec4 gValue = texture2D(tex, tex_coord);
    vec4 bValue = texture2D(tex, tex_co_modified_g);  

    frag_color = vec4(rValue.r, gValue.g, bValue.b, 1.0);
